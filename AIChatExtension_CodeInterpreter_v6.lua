...
local function attach(win, opt)
    ...
    local function send()
        ...
        -- Build request body with optional code interpreter
        local requestBody = {
            model = model,
            messages = base
        }
        ...
        elseif not result.Body then
            ...
            elseif not data.choices[1].message.content then
                ...
                if data.choices[1].message.tool_calls then
                    content = content .. "\n\n[Code Interpreter Used]"
                    for _, tool_call in ipairs(data.choices[1].message.tool_calls) do
                        if tool_call.type == "code_interpreter" then
                            content = content .. "\n```python\n" .. (tool_call.code_interpreter and tool_call.code_interpreter.input or "code executed") .. "\n```"
                        end
                    end
                end
