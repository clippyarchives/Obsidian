...
-- Load SaveManager and ThemeManager
...
local function attach(win, opt)
    ...
    ThemeManager:SetFolder("AIChatExtension")
    SaveManager:SetFolder("AIChatExtension/configs")
    ...
    -- Services Tab with save functionality
    ...
    local classInput = svcBoxRight:AddInput("svc_classes", { 
        Text = "Class Filter(s) e.g. Model, Part"; 
        Default = ""; 
        Finished = true; 
    ...
    local nameInput = svcBoxRight:AddInput("svc_names", { 
        Text = "Name Contains (comma list)"; 
        Default = ""; 
        Finished = true; 
        ClearTextOnFocus = false; 
        Callback = function(v) 
    ...
    for _, svc in ipairs(game:GetChildren()) do
        ...
        svcBoxLeft:AddToggle("SVC_"..name, {
            Text = name,
            Default = false,
            Callback = function(v)
    ...
    -- Settings Tab with save/load functionality
    local settingsTab = win:AddTab("Settings", "settings")
    local settingsLeft = settingsTab:AddLeftGroupbox("AI Settings")
    local settingsRight = settingsTab:AddRightGroupbox("Configuration")

    settingsLeft:AddInput("APIKey", {
        Default = key,
        Text = "OpenAI API Key",
    ...
    settingsLeft:AddDropdown("AIModel", {
        Values = { "gpt-4", "gpt-4-turbo", "gpt-5", "gpt-3.5-turbo" },
        Default = model,
        Text = "AI Model",
        Callback = function(v)
            model = v
    ...
    settingsLeft:AddInput("SystemPrompt", {
        Default = sys,
        Text = "System Prompt",
        Tooltip = "System message for the AI",
        Callback = function(v)
            sys = v
        end
    })

    settingsLeft:AddToggle("AutoSave", {
        Text = "Auto Save Settings",
        Default = true,
        Tooltip = "Automatically save settings on change"
    })
    ...
    -- Load autoload config at the end
