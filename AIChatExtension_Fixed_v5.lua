...
local function style_get_ai_prefix(model)
	local s = rawget(getgenv(), "obs_chat_style")
	local use_model = (s and s.use_model_prefix ~= false)
	if use_model then
		return "["..tostring(model).."] > "
	end
	local lbl = (s and s.ai_label) or "AI"
	return lbl.." > "
end

local function style_apply_textcolor(gui, who)
	local s = rawget(getgenv(), "obs_chat_style")
	if not s then return end
	local col = (who=="ai" and s.ai_color) or (who=="user" and s.user_color)
	if typeof(col) == "Color3" then gui.TextColor3 = col end
end

local function add_lbl(parent, txt, who)
...
local function add_line_with_prefix(parent, prefix_kind, body, model)
    ...
    if prefix_kind == "ai" then
        txt = style_get_ai_prefix(model)..(body or "")
    else
        local dn = "user"
    ...
    local l = add_lbl(parent, txt, prefix_kind)
    return l
end
