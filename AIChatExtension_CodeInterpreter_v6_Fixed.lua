...
local function attach(win, opt)
    ...
    local rules = {
        sys,
        "return all code inside fenced code blocks (```lua ... ``` or ```python ... ```). also tell the user that scripts are saved in the 'AI Scripts' tab where they can insert them into the IDE. you have access to python for calculations, data analysis, and plotting."
    }
    ...
    ctoggle.Text = enable_code_interpreter and "python: on" or "python: off"
    ...
    local function send()
        ...
        -- Build request body - use prompt engineering instead of tools API
        local requestBody = {
            model = model,
            messages = base
        }
        
        -- Add python instruction to system message if enabled
        if enable_code_interpreter then
            local python_rule = "When asked to solve mathematical problems, perform data analysis, create plots, or run calculations, write and execute Python code. Show both the code and the results. You can use libraries like numpy, matplotlib, pandas, etc."
            table.insert(requestBody.messages, 1, { role = "system", content = python_rule })
        end
