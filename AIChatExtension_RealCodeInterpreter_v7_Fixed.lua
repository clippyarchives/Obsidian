...
local function add_code_block(gui, code)
    ...
    if first and (#first<=5) and (first:lower()=="lua" or first:lower()=="luau" or first:lower()=="python" or first:lower()=="py") then
        t = t:gsub("^%s*[%w%-_]*\n", "", 1)
    end
...
local function attach(win, opt)
    ...
    local model = opt.model or "gpt-4.1"
    local sys = opt.system or "you are a helpful assistant"
    local ide = opt.ide
    local enable_code_interpreter = opt.code_interpreter ~= false

    local rules = {
        sys,
        "return all code inside fenced code blocks (```lua ... ``` or ```python ... ```). also tell the user that scripts are saved in the 'AI Scripts' tab where they can insert them into the IDE"
    }
    ...
    local function build_instructions()
        local inst = {}
        for _,r in ipairs(rules) do table.insert(inst, r) end
        local ctx = build_service_context(services_selected, filter)
        if ctx ~= "" then table.insert(inst, "services context:\n"..ctx) end
        for instObj, state in pairs(include_game_scripts) do
            if state and scripts_store[instObj] then
                local path = instance_path(instObj)
                table.insert(inst, "game script: "..path.."\n```lua\n"..scripts_store[instObj].."\n```")
            end
        end
        return table.concat(inst, "\n\n")
    end
    ...
    -- add code interpreter toggle
    local ctoggle = Instance.new("TextButton")
    ctoggle.BackgroundColor3 = lib.Scheme.MainColor
    ctoggle.BorderColor3 = lib.Scheme.OutlineColor
    ctoggle.Text = enable_code_interpreter and "code: on" or "code: off"
    ctoggle.FontFace = lib.Scheme.Font
    ctoggle.TextSize = 12
    ctoggle.TextColor3 = enable_code_interpreter and Color3.fromRGB(0,255,0) or lib.Scheme.FontColor
    ctoggle.Size = UDim2.new(0,80,0,20)
    ctoggle.Position = UDim2.new(1,-80,0,-25)
    ctoggle.Parent = rrow
    
    ctoggle.MouseButton1Click:Connect(function()
        enable_code_interpreter = not enable_code_interpreter
        ctoggle.Text = enable_code_interpreter and "code: on" or "code: off"
        ctoggle.TextColor3 = enable_code_interpreter and Color3.fromRGB(0,255,0) or lib.Scheme.FontColor
    end)
    ...
    local function send()
        ...
        -- Use proper Responses API for Code Interpreter
        local requestBody = {
            model = model,
            input = q,
        ...
        if enable_code_interpreter then
            requestBody.tools = {
                {
        ...
        local success, result = pcall(function()
            return request({ 
                ...
                Body = hs:JSONEncode(requestBody); 
            })
        end)
        ...
        elseif not result.Body then
            ...
            elseif data.error then
                ...
                -- Parse Responses API format properly
                local content_parts = {}
                
                -- Check for output array (new format)
                        ...
                        if item.type == "message" and item.content then
                            for _, content in ipairs(item.content) do
                                if content.type == "output_text" and content.text then
                                    table.insert(content_parts, content.text)
                                end
                            end
                        elseif item.type == "code_interpreter_call" and item.code_interpreter then
                            if item.code_interpreter.input then
                                table.insert(content_parts, "\n```python\n" .. item.code_interpreter.input .. "\n```")
                            end
                            if item.code_interpreter.outputs then
                                for _, output in ipairs(item.code_interpreter.outputs) do
                                    if output.type == "logs" and output.logs then
                                        table.insert(content_parts, "\nOutput: " .. output.logs)
                                    end
                                end
                            end
                ...
                -- Fallback to output_text (older format)
                elseif data.output_text then
                    table.insert(content_parts, data.output_text)
                else
                    out = "no output in response\nraw: " .. result.Body
                end
                
                if #content_parts > 0 then
                    out = table.concat(content_parts, "\n")
                end
            end
        end
